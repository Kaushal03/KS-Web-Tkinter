...
f2.pack(side=RIGHT)
########################################################################################################################
lblInfo=Label(Tops,font=("arial",30,"bold"),text="WELCOME TO NUMBER GUESSING GAME",fg="Steel Blue",bd=10,anchor="w")
lblInfo.pack()
...
def start():
        ...
        lblInst = Label(window, text = "Guess a number from 0 to 9")
        lblLine0 = Label(window, text = "*********************************************************************")
        lblNoGuess = Label(window, text = "No of Guesses: 0")
        lblMaxGuess = Label(window, text = "Max Guess: 3")
        lblLine1 = Label(window, text = "*********************************************************************")
        lblLogs = Label(window, text="Game Logs")
        lblLine2 = Label(window, text = "*********************************************************************")

        # create the buttons
        buttons = []
        for index in range(0, 10):
            button = Button(window, text=index, command=lambda index=index : process(index), state=DISABLED)
        ...
        def process(i):
            ...
            if guess == secretNumber:
                lbl = Label(window, text="Your guess was right. You won! :) ", fg="green")
                ...
                if guess > secretNumber:
                    lbl = Label(window, text="Secret number is less than your current guess :)", fg="red")
                    lbl.grid(row=guess_row, column=0, columnspan=5)
                    lblLogs.append(lbl)
                    guess_row += 1
                else:
                    lbl = Label(window, text="Secret number is greater than your current guess :)", fg="red")
                    lbl.grid(row=guess_row, column=0, columnspan=5)
                    lblLogs.append(lbl)
                    guess_row += 1
